...
* **Polimorfismo:** La clase gestora **`Vivero`** no utiliza arreglos separados. En su lugar, implementa un único **Inventario** de apuntadores a la clase base: `Producto* Inventario[MAX_INVENTARIO]`. Esto permite almacenar plantas y macetas juntas.
...
| **`Vivero.h`** | Manager Class. Almacena las colecciones mediante arreglos de apuntadores y ejecuta la lógica de ventas automatizada. |
...
* **Nomenclatura:** Uso de **`PascalCase`** para la definición de clases y **`camelCase`** para nombres de funciones, variables y atributos.
* **Legibilidad:** Restricción de **máximo 80 caracteres por línea** en el código fuente para facilitar su lectura y revisión.
* **Documentación:** Todo el proyecto se encuentra documentado bajo el estándar **Javadoc/Doxygen** (`/** ... */`), detallando explícitamente el propósito de cada función, sus parámetros (`@param`) y sus valores de retorno (`@return`).
...
* **Sin Interfaz Gráfica:** Está diseñado exclusivamente para operar a través de la terminal de comandos de forma eficiente.
...
* **Sin Acentos:** No utilizar caracteres especiales ni acentos para prevenir errores de codificación en la salida estándar de la consola.
